...
class OpenAI(LLM):
    ...
    def __init__(self, model=None, caching=True, max_retries=5, max_calls_per_min=60,
                 ...
                 token=None, endpoint=None, encoding_name=None, retry_exponent_base=None, retry_exponent_max=None):
        super().__init__()
        ...
        self.retry_exponent_base = retry_exponent_base
        self.retry_exponent_max = retry_exponent_max
    ...
    async def _library_call(self, **kwargs):
        ...
        assert openai.api_key is not None, (
            "You must provide an OpenAI API key to use the OpenAI LLM. "
            "Either pass it in the constructor, set the OPENAI_API_KEY environment variable, "
            "or create the file ~/.openai_api_key with your key in it."
        )
        ...
        if self.chat_mode:
            ...
            del kwargs['echo']
            del kwargs['logprobs']
            out = await openai.ChatCompletion.acreate(**openai_kwargs, **kwargs)
            out = add_text_to_chat_mode(out)
        else:
            out = await openai.Completion.acreate(**openai_kwargs, **kwargs)
            ...
            while try_again:
                try:
                    ...
                    if fail_count > self.llm.max_retries:
                        raise TooManyErrorsException(f"Too many (more than {self.llm.max_retries}) OpenAI API errors in a row!") from e

                    if isinstance(e, openai.error.RateLimitError):
                        remaining_requests = e.headers.get('x-ratelimit-remaining-requests')
                        remaining_tokens = e.headers.get('x-rate-limit-remaining-tokens')

                        # for remaining requests 1s is enough
                        if remaining_requests is not None and remaining_requests.isnumeric():
                        ...
                        # do exponent retry
                        if self.llm.retry_exponent_base is not None:
                            retry_exponent = self.llm.retry_exponent_base**fail_count
                            if self.llm.retry_exponent_max is not None:
                                retry_exponent = min(retry_exponent, self.llm.retry_exponent_max)

                            await asyncio.sleep(retry_exponent)
                            continue

                    await asyncio.sleep(3) # default case
